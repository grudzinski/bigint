...
func TestExpressionExamples(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: big.NewInt(((11 + 6) * (19 - 4) / 10) % 9),
		},
		{
			name:     "bitwise",
			expr:     "(a & b) | (c ^ d) &^ mask",
			...
			expected: big.NewInt((12 & 10) | ((5 ^ 3) &^ 2)),
		},
		{
			name:     "shift",
			expr:     "(a << n) + (b >> n)",
		...
		},
		{
			...
			expected: big.NewInt(abs(-12) + sqrt(81)),
		},
		{
			...
			expected: big.NewInt(modInverse(3, 11) + rem(-20, 6) + quo(-20, 6)),
		},
		{
			name:     "mixed",
